  ...
  <div class="app-title">
    ...
    <ul class="app-breadcrumb breadcrumb d-none d-lg-flex">
      <li class="mx-4">
        <h6 class="mb-0">
          TOTAL:  
          <span class="text-success font-italic" id="cantGerentes">&nbsp;<?= $data['cantidadGerentes']; ?></span>
        </h6>
      </li>
    </ul>
  </div>
          ...
          <div class="col-md-12">
            <div class="tile">
              <div class="tile-body">
                ...
                <table class="table table-striped text-center" id="tableGerentes">
                  <thead>
                    <tr>
                      <th>Matrícula</th>
                      <th>Nome</th>
          ...
          <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-mensal" role="tabpanel" aria-labelledby="nav-mensal-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="date-picker gerentesMes" name="gerentesMes" placeholder="Mês e Ano">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchGerentesMes()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaMesGerentes"></div>
            </div>
            <div class="tab-pane fade" id="nav-anual" role="tabpanel" aria-labelledby="nav-anual-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="gerentesAnio" name="gerentesAnio" placeholder="Ano" minlength="4" maxlength="4" onkeypress="return controlTag(event);">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchGerentesAnio()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaAnioGerentes"></div>
            </div>
          </div>
        </div>
    </div>
...
<script>
  ...
  let mes = '<?= $data['gerentesMDia']['numeroMes']; ?>';
  let ano = '<?= $data['gerentesMDia']['anio']; ?>';
  ...
  Highcharts.chart('graficaMesGerentes', 
  {
    ...
    xAxis: {
        categories: [
          <?php 
            foreach ($data['gerentesMDia']['usuarios'] as $dia) {
              echo $dia['dia'].",";
            }
          ?>
        ]
    ...
    series: [{
        ...
        data: [
          <?php 
            foreach ($data['gerentesMDia']['usuarios'] as $usuario) {
  ...
  Highcharts.chart('graficaAnioGerentes', 
  {
    ...
    series: [{
        ...
        data: [
          <?php 
            foreach ($data['gerentesAnio']['meses'] as $mes) {
