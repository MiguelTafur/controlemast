  ...
  <div class="app-title">
    ...
    <ul class="app-breadcrumb breadcrumb d-none d-lg-flex">
      <li class="mx-4">
        <h6 class="mb-0">
          TOTAL:  
          <span class="text-success font-italic" id="cantGestores">&nbsp;<?= $data['cantidadGestores']; ?></span>
        </h6>
      </li>
    </ul>
  </div>
          ...
          <div class="col-md-12">
            <div class="tile">
              <div class="tile-body">
                ...
                <table class="table table-striped text-center" id="tableGestores">
                  <thead>
                    <tr>
                      <th>Matrícula</th>
                      <th>Nome</th>
          ...
          <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-mensal" role="tabpanel" aria-labelledby="nav-mensal-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="date-picker gestoresMes" name="gestoresMes" placeholder="Mês e Ano">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchGestoresMes()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaMesGestores"></div>
            </div>
            <div class="tab-pane fade" id="nav-anual" role="tabpanel" aria-labelledby="nav-anual-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="gestoresAnio" name="gestoresAnio" placeholder="Ano" minlength="4" maxlength="4" onkeypress="return controlTag(event);">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchGestoresAnio()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaAnioGestores"></div>
            </div>
          </div>
        </div>
    </div>
...
<script>
  ...
  let mes = '<?= $data['gestoresMDia']['numeroMes']; ?>';
  let ano = '<?= $data['gestoresMDia']['anio']; ?>';
  ...
  Highcharts.chart('graficaMesGestores', 
  {
    ...
    xAxis: {
        categories: [
          <?php 
            foreach ($data['gestoresMDia']['usuarios'] as $dia) {
              echo $dia['dia'].",";
            }
          ?>
        ]
    ...
    series: [{
        ...
        data: [
          <?php 
            foreach ($data['gestoresMDia']['usuarios'] as $usuario) {
  ...
  Highcharts.chart('graficaAnioGestores', 
  {
    ...
    series: [{
        ...
        data: [
          <?php 
            foreach ($data['gestoresAnio']['meses'] as $mes) {
