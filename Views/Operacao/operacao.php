          ...
          <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active" id="nav-mensal" role="tabpanel" aria-labelledby="nav-mensal-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="date-picker operadoresMes" name="operadoresMes" placeholder="Mês e Ano">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchOperadoresMes()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaMesOperadores"></div>
            </div>
            <div class="tab-pane fade" id="nav-anual" role="tabpanel" aria-labelledby="nav-anual-tab">
              <div class="container-title">
                <div class="dflex">
                  <input class="operadoresAnio" name="operadoresAnio" placeholder="Ano" minlength="4" maxlength="4" onkeypress="return controlTag(event);">
                  <button type="button" class="btn btn-info btn-sm" onclick="fntSearchOperadoresAnio()">
                    <i class="fas fa-search" title="Procurar data"></i>
                  </button>
                </div>
              </div>
              <div id="graficaAnioOperadores"></div>
            </div>
          </div>
        </div>
    </div>
...
<script>
  ...
  Highcharts.chart('graficaMesOperadores', 
  {
    ...
    xAxis: {
        categories: [
          <?php 
            foreach ($data['operadoresMDia']['usuarios'] as $dia) {
              echo $dia['dia'].",";
            }
          ?>
        ]
  ...
  //Ano
  Highcharts.chart('graficaAnioOperadores', 
  {
    chart: {
        type: 'column'
    },
    ...
    series: [{
        ...
        data: [
          <?php 
            foreach ($data['operadoresAnio']['meses'] as $mes) {
