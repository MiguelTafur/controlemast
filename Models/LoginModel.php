<?php 
	...
	class LoginModel extends Mysql
	{
		...
		public function loginUser(string $ruta, string $codigo, string $usuario)
		{
			$this->strRuta = $ruta;
			$this->intCodigo = $codigo;
			$this->strUsuario = $usuario;
			$sql = "SELECT pe.idpersona, pe.rolid ,pe.status, ru.idruta, ru.codigo, ru.nombre 
					FROM persona pe INNER JOIN ruta ru ON(pe.codigoruta = ru.idruta) 
					WHERE ru.nombre = '$this->strRuta' AND 
						  ru.codigo = '$this->intCodigo' AND 
						  pe.email_user = '$this->strUsuario' AND 
						  pe.status != 0";
